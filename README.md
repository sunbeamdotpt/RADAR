...
- **REST API** (`src/server/`) — read-only JSON over the latest stored report.
...
scripts/dev-job.sh    # one inventory pass (clone sbbb, check upstreams, save)
...
deno task serve                       # serves that file on :8080
...
- **Pipeline gating** — the API's `update_available` flags and the Postgres store let pipelines gate
  releases, open upgrade tickets, or feed release notes. Example:
